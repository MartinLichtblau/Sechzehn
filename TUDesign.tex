...
\subsection{ChatKit}
ChatKit for Android \cite{chatkit} is a library that is designed to simplify the UI development of chats in Android. It offers predefined components like Dialog Lists, Message Lists, and Message Input. Furthermore it is highly extensible which we used to  provide state-of-the-art messaging features like read receipts without too much additional effort.
...
\begin{enumerate}
\item Permission to take pictures and videos: This permission allows us to take picturesL and add them to venues.
...
The user receives push notifications if a new message is sent to him. For this it is not required to have the app open.
\item Web Interface: \\
We also provide a web interface to confirm email addresses and set new passwords.
\item Live Updates: \\
The location is updated every 100m to provide a near to real-time feeling. This happens in the background even if the user interacts with another app or has no app open at all.
\item Encryption: \\
...
By sliding up the quick search bar, the extensive search view shown in \autoref{fig:search_view} gets visible.
